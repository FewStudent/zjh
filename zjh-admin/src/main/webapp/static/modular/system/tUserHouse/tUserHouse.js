 ...
 * 初始化表格的列
 */
TUserHouse.initColumn = function () {
    return [
 ...
 * 检查是否选中
 */
TUserHouse.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if (selected.length == 0) {
        Feng.info("请先选中表格中的某一记录！");
        return false;
    } else {
        TUserHouse.seItem = selected[0];
        return true;
    }
};
 ...
 * 点击添加
 */
TUserHouse.openAddTUserHouse = function () {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/tUserHouse/tUserHouse_add'
    });
    layer.full(index);
 ...
 * 打开查看详情
 */
TUserHouse.openTUserHouseDetail = function () {
    if (this.check()) {
        ...
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/tUserHouse/tUserHouse_update/' + TUserHouse.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
...
TUserHouse.detail = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/tUserHouse/tUserHouse_detail/' + TUserHouse.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
...
TUserHouse.examine = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/tUserHouse/examine/' + TUserHouse.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
...
TUserHouse.end = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/tUserHouse/end/' + TUserHouse.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
...
TUserHouse.make = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/tUserHouse/make/' + TUserHouse.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
 ...
 * 删除
 */
TUserHouse.delete = function () {
    if (this.check()) {
        var oid = this.seItem.id;
        layer.confirm("确定删除？", {title: '删除确认', btn: ['确定', '取消']}, function (index) {
            layer.close(index);
            var ajax = new $ax(Feng.ctxPath + "/tUserHouse/delete", function (data) {
                Feng.success("删除成功!");
                TUserHouse.table.refresh();
            }, function (data) {
                Feng.error("删除失败!" + data.message + "!");
            });
            ajax.set("tUserHouseId", oid);
            ajax.start();
        }, function () {
            layer.msg("取消删除操作");
        });
...
TUserHouse.search = function () {
    ...
    TUserHouse.table.refresh({query: queryData});
};

TUserHouse.reset = function () {
    $("#end").val(-1);
    $("#status").val(-1);
    $('#filed1').val("");
...
$(function () {
    var defaultColunms = TUserHouse.initColumn();
    var table = new BSTable(TUserHouse.id, "/tUserHouse/list", defaultColunms);
    table.setPaginationType("client");
    TUserHouse.table = table.init();
});
